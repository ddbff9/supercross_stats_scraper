...
def getResultsLinks(dbColumn1, dbColumn2, dbColumn3, dbColumn4, dbTable, eventSeason):
    ...
    query = "SELECT " + dbColumn1 + ',' + dbColumn2 + ',' + dbColumn3 + ',' + dbColumn4 + \
            ...
            " WHERE eventID like '" + \
        str(eventSeason) + "%' AND sessionName = 'Provisional Results';"
        ...
        for table in prov_results_html:
            ...
            for result in prov_table:
                ...
                if len(provPos_parts) < 2:
                    ...
                    prov_results_list = [
                        eventID, raceID, sessionID, sessionTitle, provPos, riderNum, riderName, riderBike]

                    # Append result data into the season_qual_results list:
                    season_prov_results.append(prov_results_list)

    # Return the list that contains all the results for the season:
    return season_prov_results
...
print("\n    This program will scrape provisional results from Supercross racing")
print("    sessions stored on SupercrossLive.com and store this info in a")
print("    SQL Database.\n")

startYear = input("        Enter Year (2013 to Present): ")

print('\n        Program is scraping the provisional results...')

season_prov_results_list = getProvResults(startYear)

print('\n        List Created, now adding provisional results to sql database, this will take a few minutes...\n')

storeResults(season_prov_results_list)

print("\n        Completed! " + str(len(season_prov_results_list)) +
